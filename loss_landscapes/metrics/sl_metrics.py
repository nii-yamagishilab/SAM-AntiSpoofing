...
class Loss(Metric):
    ...
    def __call__(self, model_wrapper: ModelWrapper) -> float:
        ...
        if isinstance(outputs, tuple):
            outputs = outputs[0]
        loss = self.loss_fn(outputs, self.target).item()
        return loss


class LossBatches(Metric):
    """ Computes a specified loss function over specified input-output pairs. """
    def __init__(self, loss_fn, dataloader, device, ratio=0.5):
        super().__init__()
        self.loss_fn = loss_fn
        self.dataloader = dataloader
        self.device = device
        self.num_batches = int(ratio * len(dataloader))
        print(f"Using {self.num_batches} batches for loss computation")
