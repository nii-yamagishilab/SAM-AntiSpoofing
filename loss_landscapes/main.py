...
def planar_interpolation(model_start: typing.Union[torch.nn.Module, ModelWrapper],
    ...
    tbar = tqdm(total=steps * steps, desc='Steps')
...
def random_plane_with_best_params(
        model: typing.Union[torch.nn.Module, ModelWrapper], metric: Metric,
        distance=1, steps=20,
        normalization='filter') -> typing.Tuple[np.ndarray, dict]:
    """
    Modified function to avoid deepcopy issues by using PyTorch state dict for parameter management.
    Returns the loss data matrix and the best model parameters corresponding to the minimum loss.
    """
    model_start_wrapper = wrap_model(model)
    ...
    # Move start point so original params are in the center of the plane
    dir_one.mul_(steps / 2)
    dir_two.mul_(steps / 2)
    start_point.sub_(dir_one)
    start_point.sub_(dir_two)
    ...
    for i in range(steps):
        ...
        for j in range(steps):
            ...
            if i % 2 == 0:
                start_point.add_(dir_two)
                current_loss = metric(model_start_wrapper)
                data_column.append(current_loss)
            else:
                start_point.sub_(dir_two)
                current_loss = metric(model_start_wrapper)
                data_column.insert(0, current_loss)


            # Save best parameters using state_dict
            if current_loss < best_loss:
                best_loss = current_loss
                best_params = copy.deepcopy(model_start_wrapper.state_dict())
                best_index = (i, j)
    ...
    tbar.close()
    best_results = {'param': best_params, 'index': best_index, 'loss': best_loss}

    return np.array(data_matrix), best_results
