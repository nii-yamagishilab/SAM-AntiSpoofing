...
class SSLModel(nn.Module):
    def __init__(self, ckpt_path='pretrained/xlsr2_300m.pt', out_dim=1024):
        super(SSLModel, self).__init__()
        arg_overrides = {
            "encoder_layerdrop": 0.0,  # Disable LayerDrop
            "dropout_features": 0.0  # Disable feature dropout
        }
...
class SSL_Linear(nn.Module):
    def __init__(self, ssl_frozen=False, **kwargs):
        super(SSL_Linear, self).__init__()
        self.ssl_model = SSLModel(**kwargs)
        if ssl_frozen:
            for param in self.ssl_model.parameters():
                param.requires_grad = False
        self.output_layer = nn.Sequential(
            nn.Linear(self.ssl_model.out_dim, 128),
            nn.ReLU(),
            nn.Dropout(0.3),
            nn.Linear(128, 2)
        )
...
class SSL_AASIST(nn.Module):
    def __init__(self, ssl_frozen=False, **kwargs):
        super(SSL_AASIST, self).__init__()
        self.ssl_model = SSLModel(**kwargs)
        if ssl_frozen:
            for param in self.ssl_model.parameters():
                param.requires_grad = False
    ...
    def forward(self, x):
        ...
        output, last_hidden = self.aasist(x)
        return output, last_hidden


class AASIST(nn.Module):
    """Revised AASIST with attention module and position encoding"""
